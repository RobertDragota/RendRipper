...
Shader::Shader(const char* vPath,const char* fPath){
    ...
    glShaderSource(vsID,1,&vSrc,nullptr); glCompileShader(vsID);
    int ok; char log[512];
    glGetShaderiv(vsID,GL_COMPILE_STATUS,&ok);
    if(!ok){ glGetShaderInfoLog(vsID,512,nullptr,log); std::cerr<<"VERTEX ERROR:\n"<<log; }
    unsigned int fsID=glCreateShader(GL_FRAGMENT_SHADER);
    glShaderSource(fsID,1,&fSrc,nullptr); glCompileShader(fsID);
    glGetShaderiv(fsID,GL_COMPILE_STATUS,&ok);
    if(!ok){ glGetShaderInfoLog(fsID,512,nullptr,log); std::cerr<<"FRAG ERROR:\n"<<log; }
    ID=glCreateProgram();
    glAttachShader(ID,vsID); glAttachShader(ID,fsID);
    glLinkProgram(ID);
    glGetProgramiv(ID,GL_LINK_STATUS,&ok);
    if(!ok){ glGetProgramInfoLog(ID,512,nullptr,log); std::cerr<<"LINK ERROR:\n"<<log; }
